...
const useMessageListPaginator = (
  ...
  const { page: pageNum, total } = shareConversationParameters;

  const setPageNum = useCallback(
    (pageOrPageSetter: number | ((preState: number) => number)) => {
      updateShareConversationParameters((preState) => {
        const page =
          typeof pageOrPageSetter === 'function'
            ? pageOrPageSetter(preState.page)
            : pageOrPageSetter;
  ...
  }, [ready, startMonitor, total, pageSize]);
  ...
  useEffect(() => {
    ...
    // 当 list.length 变化时，代表滚动加载了
    // 需要把滚动位置移动到 lastTimeObserverTarget.current 的位置
    const scrollContainer = getScrollContainerElement();
    console.log(`zztest scrollTop.current`, scrollTop.current);
    console.log(`zztest currentScrollHeight`, scrollContainer?.scrollHeight);
    console.log(
      `zztest originalScrollHeight.current`,
      originalScrollHeight.current,
    );
