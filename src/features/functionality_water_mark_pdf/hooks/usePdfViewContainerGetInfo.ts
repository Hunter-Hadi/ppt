...
const useChatPdfContainerGetInfo = (props:{pdfFile:File}) => {
  ...
  const [pdfDocument, setPdfDocument] = useState<any | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    setPdfTextContentList([]);
    currentPdfDocument.current = null;
    if (currentRollingPageLoadTask.current) {
      currentRollingPageLoadTask.current.cancel();
      currentRollingPageLoadTask.current = null;
    }

    setPDFNumPages(0);
    setPdfInfoList([]);
    setPdfSizeList([]);
    pdfIsLoadingObj.current = {};
  }, []);
  ...
  // debugger    
    const loadingTask = pdfjs.getDocument(currentPdfUint8Array);
    ...
    try {
      ...
      console.log('An error occurred while loading the PDF:', error);
    } finally {
      // 确保在完成或取消后清除 currentTask
      if (currentTask === loadingTask) {
        currentTask = null;
      }
    }
  }
  const getFilePdfDocument = async (
    pdfFile: File,
  ) => {
    //根据文件获取PdfDocument开始显示
    try {
      // setIsPdfLoading(true);
      const pdfDocument = await loadPdfDocument(pdfFile);
      if (pdfDocument) {
        ...
        // initPdfInfo(pdfDocument, conversationId);
  ...
  //       setPDFNumPages(pdfNumPages);
  //       let PDFContent = '';
  //       const allTextContentList: TextContent[] = [];
  //       for (let PDFIndex = 1; PDFIndex <= pdfNumPages; PDFIndex++) {
  //         if (conversationId !== currentConversationId.current) return;
  //         if (pdfInfoList[PDFIndex - 1]) continue;
  //         setLoadingProgress((PDFIndex / pdfNumPages) * 100);
  //         const page = await pdfDocument.getPage(PDFIndex);
  //         const textContent = await page.getTextContent();
  //         allTextContentList.push(textContent);
  //         const viewport = page.getViewport({ scale: pdfPageClarity });
  //         if (conversationId !== currentConversationId.current) return;
  //         if (!props.isHaveSystemPrompt) {
  //           //如果没有系统提示的话，就把所有的文本拼接起来，用于summary
  //           let pageTextContent = '';
  //           textContent.items.forEach((item) => {
  //             const textItem = item as TextItem;
  //             pageTextContent += textItem.str;
  //             if (textItem.hasEOL) {
  //               pageTextContent += '\n';
  //             }
  //           });
  //           PDFContent += pageTextContent;

  //           if (pdfNumPages === PDFIndex) {
  //             //最后一页的时候把所有的文本拼接起来
  //             props.onAllText(PDFContent);
  //           }
  //         }
  //         if (pdfNumPages === PDFIndex) {
  //           console.log('pdfchat 读取文字完成', allTextContentList);
  //           setPdfTextContentList(allTextContentList);
  //           setIsPdfLoading(false);
  //         }
  //         console.log('initPdfInfo 当前读取页数：', PDFIndex);
  //         if (conversationId !== currentConversationId.current) return;
  //         setPdfInfoList((prev) => {
  //           let newList = [...prev];
  //           if (newList.length === 0) {
  ...
  //     }
  //   } catch (e) {
  //     console.log('pdfchat initPdfInfo error:', e);
  //   }
  // };
