...
export const isTargetTestPathname = (
  pathname: string,
  targetPathname: string | Array<string | ((pathname: string) => boolean)>,
) => {
  if (Array.isArray(targetPathname)) {
    return targetPathname.some((pathOrFunction) => {
      if (typeof pathOrFunction === 'function') {
        return pathOrFunction(removeLocaleInPathname(pathname))
      } else {
        return (
          pathname === pathOrFunction ||
          removeLocaleInPathname(pathname) === pathOrFunction
        )
      }
    })
  } else {
